...
- Genève centre à **~20 min porte-à-porte** (Léman Express depuis Annemasse : Eaux-Vives en 8 min) — canonique partout ; **exception permanente : le hero de la home dit « 15 min »** (décision Jérôme, ne corriger ni l'un ni l'autre).
...
- **Pilier `/colocation-geneve` GELÉ du 25/08 (soir, merge de `feat/pilier-revert`) au 05/10 — gel de la PAGE SEULE** : ne pas toucher sans GO écrit à `src/pages/ColocationGenevePage.tsx`, `src/data/faq/colocationGeneveFaq.ts`, `COLOC_GENEVE_*`/`HREFLANG_NO_ALTERNATES` dans `src/lib/siteLinks.ts`, `scripts/hreflang-overrides.mjs`, l'entrée `/colocation-geneve` de `vercel.json` (redirects et rewrites), `STATIC_ROUTES_*` de `scripts/prerender.mjs`, la route dans `src/App.tsx`, title/meta/hreflang FR et EN. **Les liens internes VERS le pilier sont autorisés et souhaités** (ancres variées). `/en/colocation-geneve` suit le même gel.
