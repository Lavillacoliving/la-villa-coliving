-- ============================================================================
-- Mise à jour : s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france (page de décision — brief « Conquête IA »)
-- Généré le 2026-09-08 par scripts/build-article-sql.mjs depuis content/decision-pages/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france.{fr,en}.md + .meta.json
...
-- Lien entrant depuis « living-in-france-working-in-geneva » (ancre FR « comment vivre en France tout en travaillant en Suisse », EN « seek housing on the French side ») — idempotent
UPDATE public.blog_posts SET
  content_fr = replace(content_fr, 'comment vivre en France tout en travaillant en Suisse', '[comment vivre en France tout en travaillant en Suisse](/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)'),
  content_en = replace(content_en, 'seek housing on the French side', '[seek housing on the French side](/en/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)'),
  updated_at = now()
WHERE slug = 'living-in-france-working-in-geneva'
  AND content_fr NOT LIKE '%](/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)%'
  AND (content_fr LIKE '%comment vivre en France tout en travaillant en Suisse%' OR content_en LIKE '%seek housing on the French side%');

-- Lien entrant depuis « ou-habiter-frontalier-suisse-villes-france-pas-cher » (ancre FR « où vivre côté France », EN « where to live on the French side ») — idempotent
UPDATE public.blog_posts SET
  content_fr = replace(content_fr, 'où vivre côté France', '[où vivre côté France](/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)'),
  content_en = replace(content_en, 'where to live on the French side', '[where to live on the French side](/en/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)'),
  updated_at = now()
WHERE slug = 'ou-habiter-frontalier-suisse-villes-france-pas-cher'
  AND content_fr NOT LIKE '%](/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)%'
  AND (content_fr LIKE '%où vivre côté France%' OR content_en LIKE '%where to live on the French side%');

-- Consolidation de « colocation-expats-geneve-guide » dans « s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france » : dépublication (JAMAIS de DELETE — runbook scripts/consolidation/00-RUNBOOK.md)
UPDATE public.blog_posts SET is_published = false, updated_at = now() WHERE slug = 'colocation-expats-geneve-guide';
-- Repointage des liens internes vers l'ancien slug (FR, EN, et liens /blog/ écrits dans l'EN)
UPDATE public.blog_posts SET
  content_fr = replace(content_fr, '](/blog/colocation-expats-geneve-guide)', '](/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)'),
  content_en = replace(replace(content_en, '](/en/blog/colocation-expats-geneve-guide)', '](/en/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)'), '](/blog/colocation-expats-geneve-guide)', '](/en/blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france)'),
  updated_at = now()
WHERE content_fr LIKE '%](/blog/colocation-expats-geneve-guide)%' OR content_en LIKE '%/blog/colocation-expats-geneve-guide)%';
-- Côté code (même PR) : node scripts/redirects.mjs --add /blog/colocation-expats-geneve-guide /blog/s-installer-a-geneve-expatrie-cote-suisse-ou-cote-france ; retirer « colocation-expats-geneve-guide » de src/data/blogIntentBuckets.ts ;
...
--   FROM blog_posts WHERE slug = 's-installer-a-geneve-expatrie-cote-suisse-ou-cote-france';
--   SELECT slug FROM blog_posts WHERE is_published AND (content_fr LIKE '%](/blog/colocation-expats-geneve-guide)%' OR content_en LIKE '%/blog/colocation-expats-geneve-guide)%');  -- attendu : 0 ligne
